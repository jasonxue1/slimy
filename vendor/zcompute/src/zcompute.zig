...
pub const Context = struct {
    ...
    fn initDevice(ctx: *Context, allocator: std.mem.Allocator, opts: InitOptions) !void {
        ...
        ctx.phys_device = for (devices[0..n_devices], 0..) |dev, device_index| {
            if (opts.vulkan_device_index) |wanted| {
                if (wanted != device_index) continue;
            }
